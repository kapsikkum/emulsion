...
const filmCard = f => html`<a class="film" href="#/film/${f.Line}">
  <div class="box">${f.Pic ? img(filmImg(f.Pic), f.Name) : html`<div class="none">${initials(f.Name)}</div>`}
    ${f.Rolls ? html`<span class="chip accent badge">${plural(f.Rolls, 'roll')}</span>` : ''}</div>
...
views.films = async (main, _, __, alive) => {
  ...
  catFilter.tab ??= mine.total ? 'mine' : 'all';
  ...
  const load = guard(async reset => {
    ...
    const q = new URLSearchParams({ q: catFilter.q, limit: 60, offset, shot: catFilter.tab === 'mine' ? 1 : 0, pic: catFilter.pic ? 1 : 0 });
...
const LABELS = {
  ...
  'End year': 'Discontinued', Distributor: 'Distributor', Ava: 'Available (0/1)', Pic: 'Box image file',
};
...
views.film = async (main, [id], _, alive) => {
  ...
  const specs = f.header.map((h, i) => [h, f.fields[i]]).filter(([h, v], i) => v && ![2, 3, 4, 11].includes(i))
    .map(([h, v]) => [label(h), h === 'Ava' ? (v === '1' ? 'Yes' : 'No') : v]);
  ...
  main.innerHTML = html`<div style="padding-top:22px"><a class="back" href="#/films">${icon('left')}Films</a></div>
    <div class="film-page" style="padding-top:12px">
      ...
      <div>
        <div class="maker">${maker || 'Unknown manufacturer'}</div>
        <h1>${name}</h1>
